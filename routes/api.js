...
// =====================================================
// ARCADE MODULE API ENDPOINTS
// =====================================================

// Helper function to calculate arcade metrics from real data
async function calculateArcadeMetrics() {
  const now = new Date();
  const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
  
  try {
    // Get active employees (not customers/partners)
    const { data: allProfiles } = await supabaseAdmin
      .from('profiles')
      .select('id, role_id, employee_status');
    
    // Filter out customer/partner roles
    const { data: roles } = await supabaseAdmin
      .from('roles')
      .select('id, name');
    
    const roleMap = {};
    roles?.forEach(r => { roleMap[r.id] = r.name?.toLowerCase() || ''; });
    
    const activeEmployees = allProfiles?.filter(p => {
      if (p.employee_status !== 'active') return false;
      const roleName = p.role_id ? roleMap[p.role_id] : '';
      return !roleName.includes('customer') && !roleName.includes('partner') && !roleName.includes('consumer') && !roleName.includes('klant');
    }) || [];
    
    // Get open tasks
    const { count: tasksOpen } = await supabaseAdmin
      .from('employee_tasks')
      .select('*', { count: 'exact', head: true })
    ...
    // Get open tickets
    const { count: ticketsOpen } = await supabaseAdmin
      .from('tickets')
      .select('*', { count: 'exact', head: true })
      .not('status', 'in', '(resolved,closed)');
    
    // Get urgent tickets
    const { count: ticketsUrgent } = await supabaseAdmin
      .from('tickets')
      .select('*', { count: 'exact', head: true })
      .eq('priority', 'urgent')
      .not('status', 'eq', 'closed');
    
    // Get revenue last 30 days (from service_line_items)
    const { data: revenueData } = await supabaseAdmin
      .from('service_line_items')
      .select('revenue_cents, cost_cents')
      .gte('created_at', thirtyDaysAgo.toISOString());
    
    const revenue_30d = revenueData?.reduce((sum, item) => sum + (item.revenue_cents || 0), 0) || 0;
    const cost_30d = revenueData?.reduce((sum, item) => sum + (item.cost_cents || 0), 0) || 0;
    const profit_30d = revenue_30d - cost_30d;
    
    // Get pending payments
    const { count: paymentsPending } = await supabaseAdmin
      .from('payments')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'pending');
    
    // Get overdue invoices
    const { count: invoicesOverdue } = await supabaseAdmin
      .from('invoices')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'pending')
    ...
    return {
      employees_total: activeEmployees.length,
      tasks_open: tasksOpen || 0,
      tasks_overdue: tasksOverdue || 0,
      tickets_open: ticketsOpen || 0,
      tickets_urgent: ticketsUrgent || 0,
      revenue_30d,
      profit_30d,
      payments_pending: paymentsPending || 0,
      invoices_overdue: invoicesOverdue || 0
    };
  } catch (error) {
    console.error('Error calculating arcade metrics:', error);
    return {
      employees_total: 0,
      tasks_open: 0,
      tasks_overdue: 0,
...
// GET /api/admin/arcade/state - Get full arcade state
router.get("/admin/arcade/state", requireAuth, isManagerOrAdmin, async (req, res) => {
  try {
    ...
    if (!room) {
      return res.status(404).json({
        success: false,
        error: 'Arcade room not found'
      });
    }
    
    // Get placements with entities
    const { data: placements } = await supabaseAdmin
      .from('arcade_placements')
      .select(`
        *,
        arcade_entities (*)
      `)
    ...
    // Get employee seats with profiles
    const { data: seats } = await supabaseAdmin
      .from('arcade_employee_seats')
      .select(`
        *,
    ...
    // Calculate metrics
    const metrics = await calculateArcadeMetrics();
    ...
    res.json({
      ...
      data: {
        ...
        unlocks: unlocks || [],
        wallet: wallet || { id: 1, currency: 'EUR', balance_cents: 0 },
        metrics
      }
    });
  } catch (error) {
    console.error('Error fetching arcade state:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Er is een fout opgetreden bij het ophalen van arcade state'
    });
  }
});

// POST /api/admin/arcade/seats/sync - Sync employee seats
router.post("/admin/arcade/seats/sync", requireAuth, isManagerOrAdmin, async (req, res) => {
  try {
    const { data: room } = await supabaseAdmin
      .from('arcade_rooms')
      .select('id')
    ...
    // Get roles to filter out customers/partners
    const { data: roles } = await supabaseAdmin
      .from('roles')
      .select('id, name');
    ...
    const existingEmployeeIds = new Set(existingSeats?.map(s => s.employee_profile_id) || []);
    
    // Find employees without seats
    const employeesNeedingSeats = activeEmployees.filter(e => !existingEmployeeIds.has(e.id));
    
    // Get desk entity ID
    const { data: deskEntity } = await supabaseAdmin
      .from('arcade_entities')
      .select('id')
      .eq('key', 'desk_basic')
      .single();
    
    // Get available desk positions (from placements)
    const { data: deskPlacements } = await supabaseAdmin
      .from('arcade_placements')
      .select('x, y')
      .eq('room_id', room.id)
      .eq('entity_id', deskEntity?.id)
      .order('x', { ascending: true })
      .order('y', { ascending: true });
    
    // Get occupied seat positions
    const { data: occupiedSeats } = await supabaseAdmin
      .from('arcade_employee_seats')
      .select('seat_x, seat_y')
      .eq('room_id', room.id);
    
    const occupiedPositions = new Set(occupiedSeats?.map(s => `${s.seat_x},${s.seat_y}`) || []);
    
    // Find free positions
    const freePositions = deskPlacements?.filter(p => !occupiedPositions.has(`${p.x},${p.y}`)) || [];
    
    // Assign seats
    const newSeats = [];
    for (let i = 0; i < Math.min(employeesNeedingSeats.length, freePositions.length); i++) {
      const employee = employeesNeedingSeats[i];
    ...
    await ActivityService.logActivity({
      adminId: req.user.id,
      type: 'arcade_seats_synced',
      severity: 'low',
      title: 'Arcade seats synchronized',
      description: `Synced ${newSeats.length} employee seats`,
      metadata: { seats_added: newSeats.length }
    }).catch(err => console.log('Activity logging failed:', err));
    
    res.json({
      success: true,
      data: {
        seats_added: newSeats.length,
        seats: newSeats
      }
    });
  } catch (error) {
    console.error('Error syncing arcade seats:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Er is een fout opgetreden bij het synchroniseren van seats'
    });
  }
});

// Helper: Check if tiles are occupied
async function checkTileOccupancy(roomId, tileX, tileY, footprintW, footprintH, excludePlacementId = null) {
  // Get all placements in the room
  const { data: placements, error } = await supabaseAdmin
    ...
    .select(`
      ...
      x,
      y,
      arcade_entities (footprint_w, footprint_h)
    `)
    .eq('room_id', roomId);
  ...
  placements?.forEach(p => {
    if (excludePlacementId && p.id === excludePlacementId) return;
    
    const entity = p.arcade_entities;
    ...
    for (let dy = 0; dy < h; dy++) {
      for (let dx = 0; dx < w; dx++) {
        const tx = p.x + dx;
        const ty = p.y + dy;
        if (!occupied[ty]) occupied[ty] = {};
        occupied[ty][tx] = true;
      }
    }
  });
  
  // Check if target tiles are available
  for (let dy = 0; dy < footprintH; dy++) {
    for (let dx = 0; dx < footprintW; dx++) {
      const tx = tileX + dx;
      const ty = tileY + dy;
      if (occupied[ty] && occupied[ty][tx]) {
        return false; // Occupied
      }
    }
  }
  
  return true; // All tiles available
}

// POST /api/admin/arcade/placements - Create placement
router.post("/admin/arcade/placements", requireAuth, isManagerOrAdmin, async (req, res) => {
  try {
    const { room_id, entity_id, entity_key, x, y, rotation = 0, z_index = 0, meta = {} } = req.body;
    
    if (!room_id || x === undefined || y === undefined) {
      return res.status(400).json({
        success: false,
        error: 'room_id, x, and y are required'
      });
    }
    
    // If entity_id not provided but entity_key is, find or create entity
    let finalEntityId = entity_id;
    if (!finalEntityId && entity_key) {
      ...
      if (existingEntity) {
        ...
        const nameMap = {
          'station_intake': 'Intake',
          'station_leadflow': 'Leadflow',
          'station_sales': 'Sales',
          'station_delivery': 'Delivery',
          'station_services': 'Services',
          'station_billing': 'Billing',
          'station_payroll': 'Payroll'
        };
        const { data: newEntity, error: entityError } = await supabaseAdmin
          .from('arcade_entities')
          .insert({
            key: entity_key,
            name: nameMap[entity_key] || entity_key,
            category: 'interactable',
            sprite_key: entity_key,
            footprint_w: 2,
            footprint_h: 2,
            is_interactable: true,
            interaction_type: 'open_route',
            interaction_payload: { route: routeMap[entity_key] || '/admin' }
          })
          .select()
          .single();
    ...
    if (!finalEntityId) {
      return res.status(400).json({
        success: false,
        error: 'entity_id or entity_key is required'
      });
    }
    ...
    // Note: Activity logging disabled for arcade placements to avoid enum issues
    // The placement is still tracked in the database
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Er is een fout opgetreden bij het aanmaken van placement'
    });
  }
});

// DELETE /api/admin/arcade/placements/:id - Delete placement
router.delete("/admin/arcade/placements/:id", requireAuth, isManagerOrAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    
    const { data: placement, error: fetchError } = await supabaseAdmin
      .from('arcade_placements')
      .select('*, arcade_entities(*)')
      .eq('id', id)
      .single();
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Er is een fout opgetreden bij het verwijderen van placement'
    });
  }
});

// PATCH /api/admin/arcade/placements/:id - Update placement
router.patch("/admin/arcade/placements/:id", requireAuth, isManagerOrAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { x, y, rotation, z_index, meta } = req.body;
    
    // Get existing placement
    const { data: existingPlacement, error: fetchError } = await supabaseAdmin
      .from('arcade_placements')
      .select(`
        *,
        arcade_entities (*)
    ...
    if (fetchError || !existingPlacement) {
      return res.status(404).json({
        success: false,
        error: 'Placement not found'
      });
    ...
    // If position is being updated, check collision
    if (x !== undefined || y !== undefined) {
      const newX = x !== undefined ? parseInt(x) : existingPlacement.x;
      const newY = y !== undefined ? parseInt(y) : existingPlacement.y;
      const entity = existingPlacement.arcade_entities;
      const footprintW = entity?.footprint_w || 1;
      const footprintH = entity?.footprint_h || 1;
      
      const isAvailable = await checkTileOccupancy(
        existingPlacement.room_id,
        newX,
        newY,
        footprintW,
        footprintH,
        id // Exclude current placement
      );
      
      if (!isAvailable) {
        return res.status(409).json({
          success: false,
          error: 'Tiles are already occupied by another placement'
        });
      }
    }
    ...
    if (z_index !== undefined) updateData.z_index = parseInt(z_index);
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Er is een fout opgetreden bij het bijwerken van placement'
    });
  }
});

// POST /api/admin/arcade/unlocks/:unlockKey/purchase - Purchase unlock
router.post("/admin/arcade/unlocks/:unlockKey/purchase", requireAuth, isManagerOrAdmin, async (req, res) => {
  try {
    const { unlockKey } = req.params;
    const { cost_cents } = req.body;
    
    if (!cost_cents || cost_cents <= 0) {
      return res.status(400).json({
        success: false,
        error: 'cost_cents is required and must be positive'
      });
    }
    ...
    if (!wallet || wallet.balance_cents < cost_cents) {
      return res.status(400).json({
        success: false,
    ...
    if (unlock.is_unlocked) {
      return res.status(400).json({
        success: false,
        error: 'Unlock already purchased'
      });
    }
    
    // Atomic transaction: deduct wallet, unlock, log transaction
    const newBalance = wallet.balance_cents - cost_cents;
    ...
    const { error: transactionError } = await supabaseAdmin
      ...
      .insert({
        ...
        meta: { unlock_key: unlockKey, unlock_id: unlock.id }
      });
    ...
    await ActivityService.logActivity({
      adminId: req.user.id,
      type: 'arcade_unlock_purchased',
      severity: 'low',
      title: 'Arcade unlock purchased',
      description: `Purchased unlock: ${unlockKey}`,
      metadata: { unlock_key: unlockKey, cost_cents }
    }).catch(err => console.log('Activity logging failed:', err));
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Er is een fout opgetreden bij het kopen van unlock'
    });
  }
});

// POST /api/admin/arcade/wallet/sync - Sync wallet to real finance
router.post("/admin/arcade/wallet/sync", requireAuth, isManagerOrAdmin, async (req, res) => {
  try {
    const now = new Date();
    const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
    
    // Get revenue from service_line_items last 30 days
    const { data: revenueData } = await supabaseAdmin
      .from('service_line_items')
    ...
    const { data: wallet, error: walletError } = await supabaseAdmin
      .from('arcade_wallet')
      .update({ balance_cents: revenue_30d })
      .eq('id', 1)
      .select()
      .single();
    ...
    const { error: transactionError } = await supabaseAdmin
      ...
      .insert({
        type: 'sync',
        amount_cents: revenue_30d,
        meta: { method: 'revenue_30d', period: '30d' }
      });
    ...
    await ActivityService.logActivity({
      adminId: req.user.id,
      type: 'arcade_wallet_synced',
      severity: 'low',
      title: 'Arcade wallet synchronized',
      description: `Synced wallet to revenue (30d): €${(revenue_30d / 100).toFixed(2)}`,
      metadata: { balance_cents: revenue_30d }
    }).catch(err => console.log('Activity logging failed:', err));
    
    res.json({
      success: true,
      data: {
        balance_cents: revenue_30d,
        balance_formatted: `€${(revenue_30d / 100).toFixed(2)}`
      }
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Er is een fout opgetreden bij het synchroniseren van wallet'
    });
  }
});
