...
function createApp({ area } = {}) {
  ...
  if (area === 'dashboard') {
    ...
    // Early shell for GET /admin and GET /admin/ – no routes/admin loaded; no auth so response is always fast
    const renderAdminShell = (req, res) => {
      try {
        res.render('admin/shell', { layout: false, bootstrapUrl: '/api/admin/dashboard-bootstrap' })
      } catch (err) {
        console.error('[ADMIN] shell render error:', err)
        res.status(500).send('Admin shell kon niet geladen worden. Probeer het later opnieuw.')
      }
    }
    app.get(['/admin', '/admin/'], (req, res, next) => {
      if (req.query.full === '1') return next()
      renderAdminShell(req, res)
    })
