...
function createApp({ area } = {}) {
  ...
  if (area === 'dashboard') {
    ...
    // Admin: light GET /admin first (no heavy routes at boot) so Vercel doesn't timeout
    console.log('📦 Loading admin (light shell + lazy routes)...')
    const authRoutes = require('../routes/auth')
    const adminApiLite = require('../routes/adminApiLite')

    const renderLightAdmin = (req, res) => {
    ...
    app.get(['/admin', '/admin/'], (req, res, next) => {
      ...
      requireAuth(req, res, () => renderLightAdmin(req, res))
    })
    ...
    function lazyLoadAdminRoutes (req, res, next) {
      ...
      // Router expects path without /admin prefix (same as app.use('/admin', router))
      const raw = (req.originalUrl || req.url || '')
      const pathOnly = raw.split('?')[0]
      const qs = raw.includes('?') ? raw.slice(raw.indexOf('?')) : ''
      let pathWithoutAdmin = '/'
      if (pathOnly === '/admin' || pathOnly === '/admin/') {
        pathWithoutAdmin = '/'
        req.url = qs ? '/' + qs : '/'
      } else if (pathOnly.startsWith('/admin/')) {
        const rest = pathOnly.slice(7)
        pathWithoutAdmin = rest ? '/' + rest : '/'
        req.url = pathWithoutAdmin + (qs || '')
      }
      req.baseUrl = '/admin'
      req.path = pathWithoutAdmin
