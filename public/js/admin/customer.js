...
(function() {
  ...
  function logLayoutDebug() {
    // Only run debug instrumentation in development mode
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
      try {
        ...
        if (grid && mainCol && sideCol && typeof fetch === 'function') {
          ...
          // Use silent fetch wrapper to prevent console errors
          const silentFetch = (url, options) => {
            return fetch(url, options).catch(() => {
              // Silently ignore connection errors
            });
          };
          silentFetch('http://127.0.0.1:7242/ingest/70d77f2c-8912-4bf5-a634-228d7daf290d',{
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({
              ...payloadBase,
